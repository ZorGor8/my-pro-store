...
function App() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log("=== ПОИСК СРАБОТАЛ! ===", inputValue);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      ...
      <header className="max-w-7xl mx-auto mb-6 md:mb-10 bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100">
        {/* ЭТАЖ 1: Лого и Корзина (Floor 1: Logo and Cart) */}
        <div className="flex justify-between items-center mb-6 md:mb-8">
          ...
          <div className="flex items-center gap-3 md:gap-4 group cursor-pointer select-none" onClick={() => navigate('/')}>
            ...
            <div className="flex flex-col -space-y-1">
              ...
              <p className="hidden xs:block text-[9px] md:text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em] ml-0.5">
          ...
          <div 
            onClick={toggleCart} 
            className="relative p-2.5 md:p-3 bg-gray-50 rounded-full cursor-pointer hover:bg-gray-100 transition-all active:scale-90 border border-gray-100"
          >
            <span className="text-xl md:text-2xl">🛒</span>
            {cart.length > 0 && (
              <span className="absolute -top-1 -right-1 bg-blue-600 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full animate-bounce">
                {cart.length}
              </span>
            )}
          </div>
        ...
        <div className="relative mb-6 md:mb-8">
          <input 
            ...
            className="w-full p-3.5 md:p-4 pl-10 md:pl-12 bg-gray-50 border border-transparent rounded-xl md:rounded-2xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm md:text-base text-gray-700 shadow-inner"
          />
          <span className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2 text-lg md:text-xl opacity-40">🔍</span>
        </div>

        {/* ЭТАЖ 3: Навигация (Floor 3: Navigation) */}
        <nav className="flex overflow-x-auto pb-2 md:pb-0 md:flex-wrap gap-2 border-t pt-4 md:pt-6 no-scrollbar">
          <button 
            onClick={() => setCategory('all')}
            className={`whitespace-nowrap px-4 md:px-5 py-2 md:py-2.5 rounded-lg md:rounded-xl text-xs md:text-sm font-bold transition-all ${
          ...
          {categories.map(cat => (
            <button 
              ...
              className={`whitespace-nowrap px-4 md:px-5 py-2 md:py-2.5 rounded-lg md:rounded-xl text-xs md:text-sm font-bold capitalize transition-all ${
